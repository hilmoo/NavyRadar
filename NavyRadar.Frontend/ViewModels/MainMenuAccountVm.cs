...
public class MainMenuAccountsVm : ObservableObject
{
    ...
    private async Task OnUpdateAccountAsync()
    {
        ...
        if (dialog.ShowDialog() == true)
        {
            ...
            try
            {
                ...
                var updatedSpec = MapModelUppToSpec(dialog.Account);
                Debug.WriteLine(updatedSpec.Password);
                if (updatedSpec.Password == CurrentAccount.Password)
                {
                    updatedSpec.Password = "";
                }

                var updatedAccount =
                    await ApiService.ApiClient.AccountsPUTAsync(dialog.Account.Id, updatedSpec);
    ...
    private static UpdateAcc MapModelUppToSpec(Shared.Models.Account model) =>
        new()
        {
            Id = model.Id,
            Username = model.Username,
